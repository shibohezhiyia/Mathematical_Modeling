...
def default_module_catalog() -> dict[str, Any]:
    entries = [
        ...
        ModuleEntry("dynamic_model_compiler", "core.dynamic_model_compiler", "compile_and_execute_model", "active", "typed_dispatch_and_problem_ir_regression", True, notes="题面→类型化 IR/候选→受限 JSON 合同路由；不执行生成源码"),
        ModuleEntry("automated_benchmark_protocol", "core.automated_benchmark", "run_automated_benchmark", "active", "generator_scorer_separation_regression", True, notes="有真值合成/解析任务的自动评分协议；按结构组计独立性，不等于现实泛化"),
        ModuleEntry("automated_benchmark_suite", "core.automated_benchmark_suite", "run_automated_benchmark_suite", "active", "four_family_structure_holdout", True, notes="代数/ODE/优化/多表的 4×N 有真值开发任务；不等于真实竞赛题准确率"),
        ModuleEntry("typed_execution_benchmark", "core.automated_benchmark_suite", "run_typed_execution_benchmark", "active", "32_case_typed_backend_smoke", True, notes="固定类型化合同的端到端执行冒烟；不等于开放世界泛化"),
        ModuleEntry("benchmark_holdout_split", "core.benchmark_holdout", "split_structure_holdout", "active", "structure_group_atomic_split", True, notes="开发/确认/最终分割不拆数学结构组；不等于真实未见题证明"),
        ...
        ModuleEntry("dynamic_model_competition", "core.model_competition", "compete_dynamic_families", "experimental", "cross_family_cegis_pareto_regression", True, notes="先执行各模型族 CEGIS，再按显式比较组竞争；缺失指标保持未决"),
        ...
        ModuleEntry("gnn_model_cegis_adapter", "core.gnn_model_cegis", "run_gnn_cegis", "experimental", "bounded_gnn_predictive_competition", True, optional_dependency=True, notes="将 GNN 交互筛查接入动态竞争；预测边不等于因果边"),
        ...
        ModuleEntry("temporal_interaction_screen", "core.causal_dag", "discover_temporal_causal_graph", "experimental", "chronological_lagged_holdout", True, notes="时滞预测交互筛查；不等于动态图因果识别"),
