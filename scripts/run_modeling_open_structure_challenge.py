"""Freeze and run the second unsupported-structure challenge exactly once."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run frozen open-structure challenge v2 once")
    parser.add_argument("--output", default="artifacts/modeling-open-structure-challenge/confirmation-20260920.json")
    parser.add_argument("--seed", type=int, default=20260920)
    parser.add_argument("--registry-dir", default="artifacts/modeling-confirmation/consumed")
    args = parser.parse_args()
    marker = reserve_modeling_confirmation(args.registry_dir, protocol_id="modeling-open-structure-challenge-v2", seed=args.seed)
    ...
    methods = ("typed-interface-probe/v4", "bounded-raw-induction/v4", "transparent-tools/v4")
    freeze = create_evaluation_freeze(ROOT, protocol_id="modeling-open-structure-challenge-v2",
                                      seed=args.seed, budget=budget, methods=methods)
    before = verify_evaluation_freeze(ROOT, freeze)
    cases = build_modeling_open_structure_challenge(seed=args.seed)
    ...
    report = run_three_arm_modeling_comparison({
        ...
        "candidate_new": {"version_id": methods[1], "source_digest": _digest([
            suite, ROOT / "core" / "dynamic_model_compiler.py", ROOT / "core" / "automatic_modeling.py", *supervision])},
    ...
    report["challenge_protocol"] = {"status": status, "seed": args.seed, "case_count": len(cases),
                                    ...
                                    "scope": "unsupported_structures_kept_out_of_candidate_library;internal_not_external"}
