"""Freeze before the first candidate execution of the v3 structure set."""
...
def main() -> int:
    ...
    parser.add_argument("--output", default="artifacts/modeling-unseen-structure/confirmation-20260921.json")
    parser.add_argument("--registry-dir", default="artifacts/modeling-confirmation/consumed")
    args = parser.parse_args()
    marker = reserve_modeling_confirmation(args.registry_dir, protocol_id="modeling-unseen-structure-confirmation-v3", seed=args.seed)
    ...
    methods = ("typed-interface-probe/v5", "bounded-raw-induction/v5", "transparent-tools/v5")
    freeze = create_evaluation_freeze(ROOT, protocol_id="modeling-unseen-structure-confirmation-v3",
                                      seed=args.seed, budget=budget, methods=methods)
    before = verify_evaluation_freeze(ROOT, freeze)
    cases = build_unseen_structure_confirmation(seed=args.seed)
    commitment = sha256(json.dumps([case.public_metadata() for case in cases], sort_keys=True,
    ...
    report["confirmation_protocol"] = {"status": status, "seed": args.seed, "case_count": len(cases),
        "structure_group_count": len({case.structure_group for case in cases}), "suite_commitment": commitment,
        "freeze": freeze, "freeze_before": before, "freeze_after": after,
        "scope": "first_candidate_execution_after_freeze;internal_generator;unsupported_structures"}
